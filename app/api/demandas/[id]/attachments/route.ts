...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await logActivity(user.id, 'anexou arquivo à demanda', 'Demandas', `${task.title} — ${file.name}`)
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await logActivity(user.id, 'removeu anexo da demanda', 'Demandas', attachment.fileName)
