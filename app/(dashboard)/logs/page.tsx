    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <form className="flex gap-3 flex-wrap">
          <select name="userId" defaultValue={userId || ''} className="input text-sm max-w-[200px]">
            <option value="">Todos os colaboradores</option>
            {users.map((u) => (
              <option key={u.id} value={u.id}>{u.name}</option>
            ))}
          </select>
          <select name="module" defaultValue={module || ''} className="input text-sm max-w-[200px]">
          ...
          <button type="submit"
            className="px-4 py-2 bg-[#0A1A3F] text-white rounded-lg text-sm font-medium hover:bg-[#06112B] transition-colors">
            Filtrar
          </button>
        </form>
                ...
                logs.map((log) => (
                  <tr key={log.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      <span className="text-xs bg-[#0A1A3F]/10 text-[#0A1A3F] px-2 py-1 rounded-full font-medium">
                        {log.module}
                      </span>
                    </td>
