...
import { CalendarDays, Filter, Kanban, List, ListChecks, Plus, Search, Sparkles, X } from 'lucide-react'
import SearchSelect from '@/components/ui/SearchSelect'
import { STAGES, extraDemandasFilterCount, type DemandasFilters, type Period } from '@/lib/demandas-core'
...
function FiltersButton({ filters, onApply, isAdmin }: { filters: DemandasFilters; onApply: (f: DemandasFilters) => void; isAdmin: boolean }) {
  ...
  const count = extraDemandasFilterCount(filters)
  ...
  const painel = (
    <div className="p-4 space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select value={draft.status} onChange={(e) => set('status', e.target.value)} className="w-full h-9 px-2 text-xs border border-gray-200 rounded-lg bg-white">
            ...
            {STAGES.filter((s) => isAdmin || s.key !== 'BACKLOG').map((s) => <option key={s.key} value={s.key}>{s.label}</option>)}
          </select>
        </div>
        <div>
      ...
      <div className="flex items-center justify-between gap-2 pt-2 border-t border-gray-100">
        <button
          type="button"
          onClick={() => { const limpo = { ...filters, period: '' as Period, status: '', tier: '', late: false }; setDraft(limpo); onApply(limpo); setOpen(false) }}
          className="text-[11px] font-semibold text-gray-500 hover:text-gray-900"
        >
          Limpar filtros
        </button>
        <button type="button" onClick={() => { onApply(draft); setOpen(false) }} className="px-4 h-9 bg-[#030A8C] text-white rounded-lg text-xs font-semibold hover:bg-[#02077a]">
          Aplicar
        </button>
      </div>
    </div>
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <>
          <div className="hidden sm:block absolute right-0 z-40 mt-1 w-[400px] bg-white border border-gray-200 rounded-xl shadow-lg" role="dialog" aria-label="Filtros">{painel}</div>
...
export function DemandasChips({
  ...
  if (filters.period) chips.push({ key: 'period', label: PERIOD_LABEL[filters.period] })
  if (filters.status) chips.push({ key: 'status', label: `Etapa: ${STAGES.find((s) => s.key === filters.status)?.label ?? filters.status}` })
  if (filters.tier) chips.push({ key: 'tier', label: `Grupo: ${TIER_LABEL[filters.tier] ?? filters.tier}` })
  if (filters.late) chips.push({ key: 'late', label: 'Atrasadas' })
...
export default function DemandasToolbar({
  filters, onChange, users, clients, view, onView, isAdmin, onNew, onImport,
}: {
  filters: DemandasFilters
    ...
    <div className="flex items-center gap-2 flex-wrap">
      <div className="relative flex-1 min-w-[160px] max-w-xs">
        ...
        <input
          ...
          placeholder="Buscar demanda, cliente ou #"
      ...
      {!isAdmin && (
        <div className="inline-flex rounded-lg border border-gray-200 bg-white p-0.5" role="group" aria-label="Escopo">
          {([[true, 'Minhas'], [false, 'Equipe']] as const).map(([mine, label]) => (
            <button
              key={label}
              type="button"
              onClick={() => onChange({ mine })}
              className={`h-8 px-3 rounded-md text-xs font-semibold transition-colors ${filters.mine === mine ? 'bg-[#030A8C] text-white' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              {label}
            </button>
          ))}
            ...
            return (
              <button
                ...
                className={`h-8 px-2.5 rounded-md text-xs font-semibold inline-flex items-center gap-1.5 transition-colors ${v.mobileOnly ? 'lg:hidden' : ''} ${
                  view === v.key ? 'bg-[#030A8C] text-white' : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
